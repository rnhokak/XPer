...
export function PartnersManager({ partners, categories }: { partners: Partner[]; categories: Category[] }) {
  ...
  const upsert = async (values: PartnerInput) => {
    ...
    try {
      await upsertMutation.mutateAsync({
        payload: editing ? { id: editing.id, ...payload } : payload,
  ...
  const remove = async (id: string) => {
    ...
    try {
      ...
      setSubmitError(error instanceof Error ? error.message : 'Failed to delete partner')
      return
    }
          ...
          <Form {...form}>
            <form className="space-y-3 rounded-lg border bg-white p-4" onSubmit={form.handleSubmit(upsert)}>
              ...
              <Button type="submit" disabled={upsertMutation.isPending}>
                {upsertMutation.isPending ? 'Đang lưu...' : editing ? 'Cập nhật đối tác' : 'Thêm đối tác'}
              </Button>
            </form>
          </Form>
              ...
              partners.map((p) => (
                <div key={p.id} className="flex items-center justify-between rounded-lg border bg-white px-3 py-2">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      Debt category: {p.category_id ? categoryLookup.get(p.category_id)?.name ?? '—' : '—'}
                    </p>
                  </div>
