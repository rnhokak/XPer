...
import { type Account, type Category } from '@/hooks/useDebtsData'
...
export function DebtPaymentForm({
  ...
  const createPaymentMutation = useMutation({
    mutationFn: async (payload: DebtPaymentInput) => {
      const res = await fetch(`${API_BASE_URL}/debts/payments`, {
        method: 'POST',
      ...
      if (!res.ok) {
        ...
        throw new Error(err.error ?? 'Không thể ghi nhận thanh toán')
      }
  ...
  const onSubmit = async (values: DebtPaymentInput) => {
    ...
    try {
      ...
      setSubmitError(error instanceof Error ? error.message : 'Không thể ghi nhận thanh toán')
      return
    }
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          ...
          <div className="text-right text-sm text-muted-foreground">
            <p>
              Còn lại: {remainingPrincipal.toLocaleString()} {currency}
            </p>
          </div>
        </div>
      </CardHeader>
            ...
            <form className="grid gap-4 md:grid-cols-[1.05fr,0.95fr]" onSubmit={form.handleSubmit(onSubmit)}>
              ...
              <div className="space-y-3 rounded-lg border bg-white p-4">
                ...
                <Button type="submit" disabled={createPaymentMutation.isPending}>
                  {createPaymentMutation.isPending ? 'Đang lưu...' : paymentLabel}
                </Button>
                <p className="text-xs text-muted-foreground">
                  Transaction cashflow sẽ tự tạo ({direction === 'borrow' ? 'expense' : 'income'}) và liên kết với
                  debt_payments.
                </p>
              </div>
            </form>
