...
export function DebtQuickCreateForm({ partners, accounts, categories, defaultAccountId, defaultCurrency }: Props) {
  ...
  const onSubmit = async (values: DebtCreateInput) => {
    ...
    try {
      await createMutation.mutateAsync(payload as DebtCreateInput)
    } catch (error) {
      setSubmitError(error instanceof Error ? error.message : 'Không thể tạo khoản vay')
      return
    }
          ...
          <Form {...form}>
            <form className="grid gap-4 md:grid-cols-[1.1fr,0.9fr]" onSubmit={form.handleSubmit(onSubmit)}>
              ...
              <div className="space-y-3 rounded-lg border bg-white p-4">
                    ...
                    <FormItem>
                      <FormControl>
                        <Textarea
                          ...
                          value={field.value ?? ''}
                          rows={8}
                          placeholder="Thông tin thêm, lịch trả, tài sản bảo đảm..."
                        />
                      </FormControl>
                ...
                <Button type="submit" disabled={createMutation.isPending || partners.length === 0}>
                  {createMutation.isPending ? 'Đang tạo...' : 'Tạo và ghi nhận cashflow'}
                </Button>
                <p className="text-xs text-muted-foreground">
                  Hệ thống sẽ tạo transaction cashflow với loại {direction === 'borrow' ? 'income (nhận tiền)' :
                  'expense (chi ra)'} và liên kết vào debt_payments.
                </p>
              </div>
            </form>
          </Form>
