...
export function CashflowExpenseChart({ transactions }: { transactions: CashflowTransaction[] }) {
  ...
  const viewWidth = Math.max(dayCount, 1) * 10;
  const step = dayCount > 0 ? viewWidth / dayCount : 10;
  const barWidth = Math.max(step * 0.7, 4);
      ...
      <CardContent className="pt-0">
        <div className="space-y-3">
          <div className="w-full">
            <div className="w-full rounded-2xl border border-slate-100 bg-white/80 p-4 shadow-inner">
              <svg
                role="img"
              ...
              {daySlots.map((slot, idx) => {
                if (idx % labelStep !== 0 && idx !== daySlots.length - 1) return null;
