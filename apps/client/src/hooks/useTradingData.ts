...
export function useTradingDashboardData() {
  return useQuery({
    queryKey: tradingDashboardQueryKey,
    queryFn: async () => {
      const [balances] = await Promise.all([getLatestBalances()])
      return { balances }
    },
  })
}
...
export function useLatestBalances() {
  return useQuery({
    queryKey: tradingBalancesQueryKey,
...
export function useBalanceSnapshots(accountIds: string[], startDate: string) {
  return useQuery({
    queryKey: tradingSnapshotsQueryKey(accountIds),
    queryFn: () => getBalanceSnapshots(accountIds, startDate),
...
export function useBalanceAccounts() {
  return useQuery({
    queryKey: balanceAccountsQueryKey,
...
export function useCreateBalanceAccount() {
  ...
  return useMutation({
    mutationFn: (values: Partial<BalanceAccount>) => createBalanceAccount(values),
    onSuccess: () => {
...
export function useUpdateBalanceAccount() {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: ({ id, values }: { id: string; values: Partial<BalanceAccount> }) =>
      updateBalanceAccount(id, values),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: balanceAccountsQueryKey })
      queryClient.invalidateQueries({ queryKey: tradingBalancesQueryKey })
    },
...
export function useDeleteBalanceAccount() {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: deleteBalanceAccount,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: balanceAccountsQueryKey })
      queryClient.invalidateQueries({ queryKey: tradingBalancesQueryKey })
    },
...
export function useCreateFunding() {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: (values: Partial<FundingRow>) => createFunding(values),
    onSuccess: () => {
...
export function useUpdateFunding() {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: ({ id, values }: { id: string; values: Partial<FundingRow> }) =>
      updateFunding(id, values),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: fundingQueryKey })
    },
  })
...
export function useCreateOrder() {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: (values: Partial<OrderRow>) => createOrder(values),
    onSuccess: () => {
...
export function useUpdateOrder() {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: ({ id, values }: { id: string; values: Partial<OrderRow> }) =>
